...
plot(x->sqrt(2*(1+x))-1, xlims=(-1,1), ylims=(-1,1), size=(250,250), legend=false, framestyle=:box, xlabel=L"x", ylabel=L"y")
...
plot(g, xlims=(-1,1), ylims=(-1,1), legend=false, framestyle=:box, size=(250,250), xlabel=L"x", ylabel=L"$Q_n^{(0,0,0,2)}(x)$", linestyle=[:solid :dash :solid :dash])